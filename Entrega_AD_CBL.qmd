...
# Primer paso
Queremos estudiar como afecta a la salud mental de los universitarios las redes sociales de manera simulada. Para ello a cada individuo le mediremos la cantidad de horas al día que pasa en las RRSS., su edad, la cantidad de seguidores promedio en las redes sociales, la cantidad de publicaciones en las redes, el sexo de los individuos, la red social más usada, la frecuencia con la que se publica en redes, la limitación del tiempo en RRSS de cada individuo de manera consciente y la frecuencia con la que las redes afectan en la vida diaria de los individuos. Se recalca que la mayoría de los datos están simulados por R, así que estos no tienen por qué corresponderse con la realidad, aunque las conclusiones se harán como si se tratase de la realidad. Además, debido a la longitud del proyecto se aclara que no se harán todos los estudios de correlación y análisis multivariante posibles debido a la gran cantidad de variables. En este estudio se han realizado, en concreto, $3$ análisis, que se consideran los más interesantes a nivel subjetivo. La motivación de dichos análisis viene explicada con cada uno el la sección $Tercer paso$.
...
## Tercera correlación
Además, vamos a ver la influencia entre si el perfil es público o privado y cuanto se limitan activamente las RRSS. Veamos primero un gráfico de barras:
```{r}
...
```
Como vemos, el p-valor en este caso es $0.4894$, que sifue siendo mayor a $0.05$, por tanto, aún con esta clasificación, podemos asegurar que no existe una relación significativa entre haber tomado alguna medida de limitación de consumo y el tipo de perfil de cada idividuo.
